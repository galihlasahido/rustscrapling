//! Chromium command-line flags and resource lists, ported verbatim from
//! `scrapling/engines/constants.py` (and `toolbelt/ad_domains.py`).

/// Resource types blocked by `disable_resources`, as Scrapling's `EXTRA_RESOURCES`.
///
/// The names are Playwright's lower-case resource types; incoming Chrome DevTools
/// Protocol names are normalised with
/// [`normalize_resource_type`](crate::browser::normalize_resource_type) first.
...
///
/// Scrapling hands these to Playwright as `ignore_default_args`; here they are removed from
/// the assembled command line instead.
pub const HARMFUL_ARGS: &[&str] = &[
    // Ignored to avoid detection and the popup crashing bug abuse:
    // https://issues.chromium.org/issues/340836884
    "--enable-automation",
...
///
/// Explanation of each switch: <https://peter.sh/experiments/chromium-command-line-switches/>
...
/// Known ad and tracker domains, blocked when `block_ads` is on.
///
/// Ported from `scrapling/engines/toolbelt/ad_domains.py`, which takes it from Peter Lowe's
/// ad and tracking server list: <https://pgl.yoyo.org/adservers/>.
/// Subdomains match too, exactly like any other entry of `blocked_domains`.
...
mod tests {
    ...
    #[test]
    fn constant_lists_have_the_python_lengths() {
        assert_eq!(EXTRA_RESOURCES.len(), 10);
        assert_eq!(HARMFUL_ARGS.len(), 5);
        assert_eq!(DEFAULT_ARGS.len(), 11);
        assert_eq!(STEALTH_ARGS.len(), 53);
        assert_eq!(AD_DOMAINS.len(), 3526);
    }

    #[test]
    fn every_flag_list_is_free_of_duplicates() {
        for list in [DEFAULT_ARGS, STEALTH_ARGS, HARMFUL_ARGS] {
    ...
    #[test]
    fn every_flag_starts_with_two_dashes() {
        for list in [DEFAULT_ARGS, STEALTH_ARGS, HARMFUL_ARGS] {
            for flag in list {
                assert!(flag.starts_with("--"), "{flag} is not a chromium flag");
            }
        }
    }

    #[test]
    fn default_and_stealth_args_do_not_carry_harmful_flags() {
        for flag in DEFAULT_ARGS.iter().chain(STEALTH_ARGS.iter()) {
            assert!(!HARMFUL_ARGS.contains(flag), "{flag} is harmful");
        }
    }

    #[test]
    fn ad_domains_are_lowercase_hostnames_without_duplicates() {
        let unique: HashSet<&&str> = AD_DOMAINS.iter().collect();
        assert_eq!(unique.len(), AD_DOMAINS.len());
